...
export const facebookLogin = (navigateToMainCallback) => async dispatch => {
  ...
  try {
  const res = await Facebook.logInWithReadPermissionsAsync('137019567015638', {
    permissions: ['public_profile', 'email', 'user_location', 'user_friends']
  });

  if (res.type === 'success') {
  const { data } = await axios.get(`https://graph.facebook.com/me?access_token=${res.token}&fields=id,name,picture,email`);
 ...
 async function saveUser({id, name, email}) {
  const res = await axios.post(`${environment.API_URL}auth`, {id, name, email});
  debugger;
  return res.data;
}
